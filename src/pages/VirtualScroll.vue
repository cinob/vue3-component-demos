...
// 模拟数据
const data = ref(Array.from({length:200}))
// 滚动容器
...
const pad = ref(200)
// 渲染节点总数 前补白 + 可视区 + 后补白
const num = computed(() => itemNum.value)
// 渲染节点开始索引
const startIndex = computed(() => {
  return translateY.value
    ? ((translateY.value / itemHeight.value) | 0) - 1
    : 0
})
// 渲染节点列表
const list = computed(() => {
  return data.value.slice(startIndex.value, startIndex.value + num.value).map((item, index) => ({i: startIndex.value + index}))
})
...
onMounted(() => {
  ...
  container.value.addEventListener('scroll', debounce((e) => {
    translateY.value = e.target.scrollTop
  }, 10))
})
